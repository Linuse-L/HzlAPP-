...
#define serverUrl @"http://www.jamesishere.me/Api/get/isOpen"

@implementation LORequestManger
+ (void)POST:(NSString *)URL params:(NSDictionary * )params  URl:(NSMutableDictionary *)URlvalue success:(void (^)(id response))success
     failure:(void (^)(AFHTTPRequestOperation *operation,NSError *error))Error
{
    NSEnumerator * URLKey = [URlvalue keyEnumerator];
    NSArray *array=[URLKey allObjects];
    ...
    for (int i=0;i<[array count];i++)
    {
        ...
        if(i==[array count]-1)
        {
            urlString =[NSString stringWithFormat:@"%@%@=%@",urlString,key,[URlvalue valueForKey:key]];
        }
        else {
            urlString =[NSString stringWithFormat:@"%@%@=%@&",urlString,key,[URlvalue valueForKey:key]];
        }
    ...
    if (![URL hasPrefix:@"http"]) {
        ...
        postStr = [NSString stringWithFormat:@"%@%@", _huazhonglong_,urlString] ;
    }
...
//    NSLog(@"getStr======%@",getStr);
    if (![URL hasPrefix:@"http"]) {
        ...
        getStr = [NSString stringWithFormat:@"%@%@", _huazhonglong_,URL] ;
    }
            ...
            failure:(void (^)(AFHTTPRequestOperation *operation,NSError *error))Error
{
    ...
    NSString *postStr = [NSString stringWithFormat:@"%@%@", _huazhonglong_,URL] ;
    NSMutableDictionary *dict = [params mutableCopy];
...
+ (void)Card:(NSString *)URL params:(NSDictionary * )params  URl:(NSMutableDictionary *)URlvalue success:(void (^)(id response))success
     failure:(void (^)(AFHTTPRequestOperation *operation,NSError *error))Error
{
    NSEnumerator * URLKey = [URlvalue keyEnumerator];
    NSArray *array=[URLKey allObjects];
    ...
    if (![URL hasPrefix:@"http"]) {
        ...
        postStr = [NSString stringWithFormat:@"%@",urlString] ;
    }
    NSMutableDictionary *dict = [params mutableCopy];
